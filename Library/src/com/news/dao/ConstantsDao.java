...
public class ConstantsDao
{
  ...
  public static ArrayList<NewsEntity> getNewsList()
  {
    ...
    for (int i = 0; i < 5; i++)
    {
      ...
      news.setSourceUrl("http://news.sina.com.cn/c/2014-05-05/134230063386.shtml");
      news.setTitle("可以用谷歌眼镜做的10件酷事：导航、玩游戏");
      List<String> urlList = new ArrayList<String>();
      if (i % 2 == 1)
      {
        String url1 = "https://www.baidu.com/img/bdlogo.png";
        String url2 = "https://www.baidu.com/img/bdlogo.png";
        String url3 = "https://www.baidu.com/img/bdlogo.png";
        news.setPicOne(url1);
        news.setPicTwo(url2);
        news.setPicThr(url3);
        news.setSourceUrl(
            "http://tech.sina.com.cn/zl/post/detail/i/2013-11-06/pid_8436571.htm?from=groupmessage&isappinstalled=0");
        urlList.add(url1);
        urlList.add(url2);
        urlList.add(url3);
      } else
      {
        news.setTitle("WZH-AA用车:智能短租租车平台");
        String url = "http://r3.sinaimg.cn/2/2014/0417/a7/6/92478595/580x1000x75x0.jpg";
        news.setPicOne(url);
        urlList.add(url);
      }
      news.setPicList(urlList);
      news.setPublishTime(Long.valueOf(i));
      news.setReadStatus(false);
      news.setSource("手机腾讯网");
      news.setSummary(
          "腾讯数码讯（编译：Gin）谷歌眼镜可能是目前最酷的可穿戴数码设备，你可以戴着它去任何地方（只要法律法规允许或是没有引起众怒），作为手机的第二块“增强现实显示屏”来使用。另外，虽然它仍未正式销售，但谷歌近日在美国市场举行了仅限一天的开放购买活动，价格则为1500美元（约合人民币9330元），虽然仍十分昂贵，但至少可以满足一些尝鲜者的需求，也预示着谷歌眼镜的公开大规模销售离我们越来越近了。");
      news.setMark(i);
      if (i == 4)
      {
        ...
        news.setSourceUrl("http://games.sina.com.cn/zl/duanpian/2014-05-21/141297.shtml");
        news.setPicOne(url);
        urlList.clear();
        urlList.add(url);
      } else
  ...
  public static ArrayList<NewsEntity> getNewsList2()
  {
    ArrayList<NewsEntity> newsList = new ArrayList<NewsEntity>();
    for (int i = 0; i < 3; i++)
    {
      ...
      news.setNewsId(i);
      news.setCollectStatus(false);
      news.setCommentNum(i + 2);
      news.setInterestedStatus(true);
      news.setLikeStatus(true);
      news.setReadStatus(false);
      news.setNewsCategory("首发");
      news.setNewsCategoryId(1);
      news.setSourceUrl("https://www.baidu.com/");
      news.setTitle("news" + System.currentTimeMillis());
      List<String> urlList = new ArrayList<String>();
      if (i % 2 == 1)
      {
        String url1 = "https://www.baidu.com/img/bdlogo.png";
