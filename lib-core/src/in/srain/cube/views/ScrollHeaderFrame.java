...
public class ScrollHeaderFrame extends RelativeLayout
{
	...
	public ScrollHeaderFrame(Context context, AttributeSet attrs, int defStyle)
	{
		...
		if (arr != null)
		{
			...
			if (arr.hasValue(R.styleable.ScrollHeaderFrame_scrollheaderframe_conent_container))
			{
				mContainerId = arr
						.getResourceId(
								R.styleable.ScrollHeaderFrame_scrollheaderframe_conent_container,
								0);
			}
			if (arr.hasValue(R.styleable.ScrollHeaderFrame_scrollheaderframe_disable))
			{
				mDisabled = arr
						.getBoolean(
								R.styleable.ScrollHeaderFrame_scrollheaderframe_disable,
								false);
			}
			arr.recycle();
		}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b)
	{
		...
		if (DEBUG)
		{
			Log.d(LOG_TAG, String.format("onLayout: current %s, %s %s %s %s",
					mCurrentPos, l, t, r, b));
		}
		...
		mContentViewContainer
				.layout(0, pos + headerHeight, w, h + headerHeight);
	}
	...
	private boolean moveTo(int to)
	{
		if (DEBUG)
		{
			Log.d(LOG_TAG, String.format("moveTo: %s %s, %s", to, mCurrentPos,
					mHeaderHeight));
		}
		if (mCurrentPos == to)
		{
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		if (DEBUG)
		{
			Log.d(LOG_TAG, String.format("onMeasure %s getMeasuredHeight: %s",
					h, mHeaderContainer.getMeasuredHeight()));
		}

		mContentViewContainer.measure(
			...
			if (DEBUG)
			{
				Log.d(LOG_TAG,
						String.format(
								...
								speed, deltaY, moveUp, canMoveUp, moveDown,
								canMoveDown));
			}
