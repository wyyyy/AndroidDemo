...
public class MultiPartRequestSender extends PostRequestSender
{
	...
	public MultiPartRequestSender(IRequest<?> request,
	...
	public void send() throws IOException
	{
		...
		HashMap<String, RequestData.UploadFileInfo> uploadFiles = mRequestData
				.getUploadFiles();
		...
		if (mPostData != null && mPostData.size() != 0)
		{
			...
			Iterator<Map.Entry<String, Object>> iterator = mPostData.entrySet()
					.iterator();
		...
		// send file to server
		Iterator<Map.Entry<String, RequestData.UploadFileInfo>> iterator = uploadFiles
				.entrySet().iterator();
		while (iterator.hasNext())
		{
			Map.Entry<String, RequestData.UploadFileInfo> item = iterator
					.next();
