...
public class Query<T> implements ICacheAble<T>
{
	...
	@Override
	public void onCacheData(CacheResultType cacheResultType, T cacheData,
			boolean outOfDate)
	{
		...
		if (outOfDate)
		{
			...
			{
				mHandler.onQueryFinish(RequestType.USE_CACHE_ANYWAY, cacheData,
						outOfDate);
			}
		} else
		{
			...
			if (mHandler != null)
			{
				mHandler.onQueryFinish(RequestType.USE_CACHE_NOT_EXPIRED,
						cacheData, true);
			}
		}
	}
