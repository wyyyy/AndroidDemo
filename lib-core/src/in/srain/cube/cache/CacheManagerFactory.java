...
public class CacheManagerFactory
{
	...
	public static CacheManager getDefault()
	{
		if (sDefault == null)
		{
			initDefaultCache(Cube.getInstance().getContext(),
					DEFAULT_CACHE_DIR, DEFAULT_CACHE_MEMORY_CACHE_SIZE,
					DEFAULT_CACHE_DISK_CACHE_SIZE);
		}
		return sDefault;
	}

	public static void initDefaultCache(Context content, String cacheDir,
			int memoryCacheSizeInKB, int fileCacheSizeInKB)
	{
		...
		sDefault = new CacheManager(content, cacheDir, memoryCacheSizeInKB,
				fileCacheSizeInKB);
	}
}