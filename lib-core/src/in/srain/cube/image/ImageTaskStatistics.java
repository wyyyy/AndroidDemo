	...
	public String getStatisticsInfo()
	{
		return String
				.format("mc=%d, fc=%d, wait_to_load=%d, check_file_cache=%d, download=%d/%dKB/s, decode=%d, wait_ui=%s, all=%d, size=%d/%d",
						mHitMemoryCache ? 1 : 0, mHitFileCache ? 1 : 0,
						getWaitForLoadTime(), getCheckFileCacheTime(),
						getDownloadTime(), getDownLoadSpeed(), getDecodeTime(),
						getWaitToPostMessage(), getTotalLoadTime(),
