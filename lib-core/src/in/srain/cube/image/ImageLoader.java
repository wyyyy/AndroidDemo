...
public class ImageLoader implements LifeCycleComponent
{
	 ...
	 */
	@Deprecated
	public ImageTask createImageTask(String url, int requestWidth,
			int requestHeight, ImageReuseInfo imageReuseInfo)
	{
		...
		ImageLoadRequest imageLoadRequest = new ImageLoadRequest(url,
				requestWidth, requestHeight, -1, imageReuseInfo);
	 ...
	 * @param imageView
	 */
	public void detachImageViewFromImageTask(ImageTask imageTask,
			CubeImageView imageView)
	{
		...
		if (imageTask.isLoading())
		{
			...
			{
				LoadImageTask task = mLoadWorkList.get(imageTask
						.getIdentityKey());
				if (task != null)
				{
					task.cancel();
	...
	public void addImageTask(ImageTask imageTask, CubeImageView imageView)
	{
		...
		{
			CLog.w(LOG_TAG,
					"ImageLoader has not been add to a Component Manager.",
					this);
		}
		LoadImageTask runningTask = mLoadWorkList.get(imageTask
				.getIdentityKey());
		if (runningTask != null)
		{
			if (imageView != null)
			{
				...
				runningTask.getImageTask().notifyLoading(mImageLoadHandler,
						imageView);
			}
			return;
		} else
	...
	public boolean queryCache(ImageTask imageTask, CubeImageView imageView)
	{
		...
		BitmapDrawable drawable = mImageProvider
				.getBitmapFromMemCache(imageTask);

		if (imageTask.getStatistics() != null)
		{
			imageTask.getStatistics()
					.s0_afterCheckMemoryCache(drawable != null);
		}
		if (drawable == null)
		{
			return false;
		...
		if (DEBUG)
		{
			CLog.d(LOG_TAG, MSG_HIT_CACHE, imageTask,
	...
	public static class LoadImageTask extends SimpleTask
	{
		...
		public void doInBackground()
		{
			...
			if (!isCancelled()
					...
					&& (mImageTask.isPreLoad() || mImageTask
							.stillHasRelatedImageView()))
			{
				try
				{
					bitmap = mImageLoader.mImageProvider.fetchBitmapData(
							mImageLoader, mImageTask,
							mImageLoader.mImageReSizer);
					if (DEBUG)
					{
						CLog.d(LOG_TAG, MSG_TASK_AFTER_fetchBitmapData, this,
								mImageTask, isCancelled());
					}
					mDrawable = mImageLoader.mImageProvider
							.createBitmapDrawable(mImageLoader.mResources,
									bitmap);
					mImageLoader.mImageProvider.addBitmapToMemCache(
							mImageTask.getIdentityKey(), mDrawable);
				} catch (Exception e)
				{
					e.printStackTrace();
		...
		public void onFinish(boolean canceled)
		{
			if (DEBUG)
			{
				CLog.d(LOG_TAG, MSG_TASK_FINISH, this, mImageTask,
						mImageLoader.mExitTasksEarly);
			}
			...
			{
				mImageTask.onLoadTaskFinish(mDrawable,
						mImageLoader.mImageLoadHandler);
			}
	...
	{
		if (LifeCycleComponentManager.tryAddComponentToContainer(this, object,
				throwEx))
		{
	...
	public ImageLoader attachToCubeFragment(CubeFragment fragment)
	{
		if (fragment != null)
		{
			if (LifeCycleComponentManager.tryAddComponentToContainer(this,
					fragment, true))
			{
				mHasBeenAddedToComponentManager = true;
			}
