...
import cn.commonhelp.utils.API8Util;
...
public class RequestParamsDebug extends SampleParentActivity
{
	...
	{
		return getAsyncHttpClient().get(this, getDefaultURL(),
				getRequestParams(), getResponseHandler());
	}

	// TODO: allow parsing multiple values for each type, maybe like
	// "type.key=value" ?
	private RequestParams getRequestParams()
	{
		...
		// fill in string map
		for (Map.Entry<String, Map<String, String>> entry : mapOfMaps
				.entrySet())
		{
			rp.put(entry.getKey(), entry.getValue());
		}
