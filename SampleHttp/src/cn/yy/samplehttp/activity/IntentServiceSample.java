...
import cn.commonhelp.utils.IntentUtil;
import cn.yy.sample.R;
import cn.yy.samplehttp.services.ExampleIntentService;
...
public class IntentServiceSample extends SampleParentActivity
{
	...
	{ ACTION_START, ACTION_RETRY, ACTION_CANCEL, ACTION_SUCCESS,
			ACTION_FAILURE, ACTION_FINISH };
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			} else if (ACTION_RETRY.equals(action))
			{
				addView(getColoredView(LIGHTBLUE, "Request retried"));
					...
					|| ACTION_SUCCESS.equals(action))
			{
				debugThrowable(
						LOG_TAG,
						(Throwable) intent
								.getSerializableExtra(ExampleIntentService.INTENT_THROWABLE));
				if (ACTION_SUCCESS.equals(action))
				{
					debugStatusCode(LOG_TAG, intent.getIntExtra(
							ExampleIntentService.INTENT_STATUS_CODE, 0));
					debugHeaders(
							LOG_TAG,
							IntentUtil.deserializeHeaders(intent
									.getStringArrayExtra(ExampleIntentService.INTENT_HEADERS)));
					byte[] returnedBytes = intent
							.getByteArrayExtra(ExampleIntentService.INTENT_DATA);
