...
public class DirectorySample extends SampleParentActivity
{
		...
		deleteTargetFile.setOnClickListener(new View.OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				if (lastResponseHandler != null)
				{
					...
					debugResponse(
							LOG_TAG,
							String.format("File was deleted? %b",
									toBeDeleted.delete()));
					debugResponse(
							LOG_TAG,
							String.format("Delete file path: %s",
									toBeDeleted.getAbsolutePath()));
				} else
				{
					debugThrowable(LOG_TAG, new Error(
							"You have to Run example first"));
				}
			}
		});
	...
	{
		lastResponseHandler = new FileAsyncHttpResponseHandler(getCacheDir(),
				cbAppend.isChecked(), cbRename.isChecked())
		{
			@Override
			public void onStart()
			{
