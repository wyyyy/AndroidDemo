...
import cn.commonhelp.utils.SecureSocketFactory;
import cn.yy.sample.R;

/**
 * This sample demonstrates the implementation of self-signed CA's and
 * connection to servers with such certificates. Be sure to read
 * 'res/raw/custom_ca.txt' for how-to instructions on how to generate a BKS file
 * necessary for this sample.
 * 
...
public class CustomCASample extends SampleParentActivity
{
	...
	{
		AsyncHttpClient
				.allowRetryExceptionClass(javax.net.ssl.SSLException.class);
	}
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		try
		{
			...
			try
			{
				...
				getAsyncHttpClient().setSSLSocketFactory(
						new SecureSocketFactory(store, STORE_ALIAS));
			} catch (IOException e)
			{
	...
	private CharSequence getReadmeText()
	{
		...
		try
		{
			InputStream stream = getResources()
					.openRawResource(R.raw.custom_ca);
