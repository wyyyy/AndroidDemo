...
public class UsePoolThreadSample extends GetSample
{
	...
	private class UsePoolThreadResponseHandler extends AsyncHttpResponseHandler
	{
				...
				final byte[] responseBody)
		{
			...
			runOnUiThread(new Runnable()
			{
				...
				public void run()
				{
					...
					{
						addView(getColoredView(
								LIGHTGREEN,
								"Request succeeded (" + statusCode
			...
			runOnUiThread(new Runnable()
			{
				...
				public void run()
				{
					...
					if (responseBody != null)
					{
						addView(getColoredView(
								LIGHTGREEN,
								"Download interrupted (" + statusCode
		...
		private File getRandomCacheFile()
		{
			...
			return new File(dir, "sample-" + System.currentTimeMillis()
					+ ".bin");
		}

		private void saveBytesOnDisk(File destination, byte[] bytes)
		{
			// TODO: Spin your own implementation to save the bytes on disk/SD
			// card.
			if (bytes != null && destination != null)
			{
				Log.d(LOG_TAG, "Saved " + bytes.length + " bytes into file: "
