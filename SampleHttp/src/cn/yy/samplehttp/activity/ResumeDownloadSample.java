...
import cn.commonhelp.http.help.RangeFileAsyncHttpResponseHandler;
...
public class ResumeDownloadSample extends SampleParentActivity
{
	...
	private File getDownloadTarget()
	{
		try
		{
			...
			{
				downloadTarget = File.createTempFile("download_", "_resume",
						getCacheDir());
			}
		} catch (IOException e)
		{
	...
	public ResponseHandlerInterface getResponseHandler()
	{
		return new RangeFileAsyncHttpResponseHandler(getDownloadTarget())
		{
					...
					Throwable throwable, File file)
			{
				...
				if (file != null)
				{
					addView(getColoredView(LIGHTGREEN, "Download interrupted ("
							+ statusCode + "): (bytes=" + file.length()
			...
			public void onSuccess(int statusCode, Header[] headers, File file)
			{
				...
				if (file != null)
				{
					addView(getColoredView(LIGHTGREEN, "Request succeeded ("
							+ statusCode + "): (bytes=" + file.length()
							+ "), path: " + file.getAbsolutePath()));
				}
			}
