...
public class PrePostProcessingSample extends SampleParentActivity
{
	...
	public AsyncHttpRequest getHttpRequest(DefaultHttpClient client,
			...
			Context context)
	{
		return new PrePostProcessRequest(client, httpContext, uriRequest,
				responseHandler);
	}
	...
	protected void debugProcessing(String TAG, String state, String message,
			final int color)
	{
		final String debugMessage = String.format(Locale.US,
				"%s-processing: %s", state, message);
	...
	private class PrePostProcessRequest extends AsyncHttpRequest
	{
		...
		public PrePostProcessRequest(AbstractHttpClient client,
