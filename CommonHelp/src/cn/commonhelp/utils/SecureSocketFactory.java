...
/**
 * A class to authenticate a secured connection against a custom CA using a BKS
 * store.
 * 
...
public class SecureSocketFactory extends SSLSocketFactory
{
	 ...
	 * 
	 * @param store
	 *            The key store holding the certificate details
	 * @param alias
			...
			UnrecoverableKeyException
	{
		...
		acceptedIssuers = new X509Certificate[]
		{ x509ca };
		...
		{ new X509TrustManager()
		{
					...
					String authType) throws CertificateException
			{
				...
				{
					Log.i(LOG_TAG, "Chain includes " + chain.length
							+ " certificates.");
					try
					{
						for (X509Certificate cert : chain)
						{
							...
							Log.i(LOG_TAG, "---------------------------");
							Log.i(LOG_TAG, "IssuerDN: "
									+ cert.getIssuerDN().toString());
							Log.i(LOG_TAG, "SubjectDN: "
									+ cert.getSubjectDN().toString());
							Log.i(LOG_TAG,
									"Serial Number: " + cert.getSerialNumber());
							Log.i(LOG_TAG, "Version: " + cert.getVersion());
							Log.i(LOG_TAG, "Not before: "
									+ cert.getNotBefore().toString());
							Log.i(LOG_TAG, "Not after: "
			...
			boolean autoClose) throws IOException
	{
		...
		Socket sslSocket = sslCtx.getSocketFactory().createSocket(socket, host,
				port, autoClose);
