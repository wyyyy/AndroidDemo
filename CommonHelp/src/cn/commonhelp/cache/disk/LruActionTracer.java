...
package cn.commonhelp.cache.disk;
...
public final class LruActionTracer implements Runnable
{
	 ...
	 */
	private final ExecutorService mExecutorService = new ThreadPoolExecutor(0,
	...
	@SuppressWarnings("rawtypes")
	public LruActionTracer(DiskCache diskCache, File directory, int appVersion,
			long capacity)
	{
	...
	public void tryToResume() throws IOException
	{
		if (mJournalFile.exists())
		{
			try
			{
				readJournal();
				processJournal();
				mJournalWriter = new BufferedWriter(new FileWriter(
						mJournalFile, true), IO_BUFFER_SIZE);
	...
	public synchronized void clear() throws IOException
	{
		...
		// abort edit
		for (CacheEntry cacheEntry : new ArrayList<CacheEntry>(
				mLruEntries.values()))
		{
	...
	public void commitEdit(CacheEntry cacheEntry) throws IOException
	{
		if (SimpleDiskLruCache.DEBUG)
		{
			CLog.d(SimpleDiskLruCache.LOG_TAG, "commitEdit: %s",
					cacheEntry.getKey());
		}
	...
	private void processJournal() throws IOException
	{
		...
		for (Iterator<CacheEntry> i = mLruEntries.values().iterator(); i
				.hasNext();)
		{
	...
	private void rebuildJournal() throws IOException
	{
		...
		Writer writer = new BufferedWriter(new FileWriter(mJournalFileTmp),
				IO_BUFFER_SIZE);
		writer.write(MAGIC);
		writer.write("\n");
		...
		for (CacheEntry cacheEntry : mLruEntries.values())
		{
			if (cacheEntry.isUnderEdit())
			{
				writer.write(sACTION_LIST[ACTION_DIRTY] + ' '
						+ cacheEntry.getKey() + " " + cacheEntry.getSize()
						+ '\n');
			} else
			{
				writer.write(sACTION_LIST[ACTION_CLEAN] + ' '
						+ cacheEntry.getKey() + " " + cacheEntry.getSize()
						+ '\n');
			}
		}
	...
	private void writeActionLog(byte action, CacheEntry cacheEntry)
			throws IOException
	{
		...
		if (mSize > mCapacity)
		{
			if (SimpleDiskLruCache.DEBUG)
			{
				CLog.d(SimpleDiskLruCache.LOG_TAG,
						"should trim, current is: %s", mSize);
			}
		}
		while (mSize > mCapacity)
		{
			Map.Entry<String, CacheEntry> toEvict = mLruEntries.entrySet()
					.iterator().next();
			...
			{
				CLog.d(SimpleDiskLruCache.LOG_TAG,
						"pending remove: %s, size: %s, after remove total: %s",
						key, cacheEntry.getSize(), mSize);
			}
		}
	}
