...
import cn.commonhelp.cache.utils.CubeDebug;
import cn.commonhelp.cache.utils.SimpleExecutor;
import cn.commonhelp.cache.utils.SimpleTask;
...
public class CacheManager
{
			...
			int memoryCacheSizeInKB, int fileCacheSizeInKB)
	{
		...
		mMemoryCache = new LruCache<String, CacheMetaData>(
		...
		DiskFileUtils.CacheDirInfo cacheDirInfo = DiskFileUtils
				.getDiskCacheDir(content, cacheDir, fileCacheSizeInKB, null);
		mFileCache = DiskCacheProvider.createLru(content, cacheDirInfo.path,
				cacheDirInfo.realSize);

		if (DEBUG)
		{
			CLog.d(LOG_TAG, "CacheManger: cache dir: %s => %s, size: %s => %s",
					cacheDir, cacheDirInfo.path, cacheDirInfo.requireSize,
					cacheDirInfo.realSize);
		}
	}
	...
	public <T> T requestCacheSync(ICacheAble<T> cacheAble)
	{
		...
		if (mRawData != null)
		{
			...
			cacheAble.onCacheData(CacheResultType.FROM_INIT_FILE, ret,
					outOfDate);
			return ret;
		}
	...
	private class InnerCacheTask<T1> extends SimpleTask
	{
		...
		void beginQuery()
		{
			...
			if (mCacheAble.cacheIsDisabled())
			{
				...
				{
					CLog.d(LOG_TAG,
							"key: %s, Cache is disabled, query from server",
							cacheKey);
				}
			...
			if (DEBUG)
			{
				CLog.d(LOG_TAG, "key: %s, cache file not exist",
		...
		public void doInBackground()
		{
			if (DEBUG)
			{
				CLog.d(LOG_TAG, "key: %s, doInBackground: mWorkType: %s",
		...
		private void beginQueryFromCacheFileAsync()
		{
			if (DEBUG)
			{
				CLog.d(LOG_TAG, "key: %s, beginQueryFromCacheFileAsync",
						mCacheAble.getCacheKey());
			}
		...
		private void beginConvertDataAsync(byte convertFor)
		{
			if (DEBUG)
			{
				CLog.d(LOG_TAG, "key: %s, beginConvertDataAsync",
						mCacheAble.getCacheKey());
			}
		...
		private void doQueryFromCacheFileInBackground()
		{
			if (DEBUG)
			{
				CLog.d(LOG_TAG, "key: %s, try read cache data from file",
						mCacheAble.getCacheKey());
			}
		...
		private void doQueryFromAssertCacheFileInBackground()
		{
			...
			{
				CLog.d(LOG_TAG,
						"key: %s, try read cache data from assert file",
						mCacheAble.getCacheKey());
			}
		...
		private void doConvertDataInBackground()
		{
			if (DEBUG)
			{
				CLog.d(LOG_TAG, "key: %s, doConvertDataInBackground",
						mCacheAble.getCacheKey());
			}
			JsonData data = JsonData.create(mRawData.getData());
		...
		private void setCurrentStatus(byte status)
		{
			...
			if (DEBUG)
			{
				CLog.d(LOG_TAG, "key: %s, setCurrentStatus: %s",
		...
		private void done()
		{
			...
			if (mResult != null)
			{
				...
				case CONVERT_FOR_ASSERT:
					mCacheAble.onCacheData(CacheResultType.FROM_INIT_FILE,
							mResult, outOfDate);
					break;
				case CONVERT_FOR_CREATE:
					mCacheAble.onCacheData(CacheResultType.FROM_CREATED,
							mResult, outOfDate);
					break;
