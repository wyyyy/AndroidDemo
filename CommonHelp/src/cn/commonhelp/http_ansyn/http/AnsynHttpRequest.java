 package cn.commonhelp.http_ansyn.http;
...
import cn.commonhelp.activity.BaseActivity;
import cn.commonhelp.http_ansyn.db.DBHelper;
import cn.commonhelp.http_ansyn.utils.Encoder;
import cn.commonhelp.http_ansyn.utils.LogMy;
import cn.commonhelp.http_ansyn.utils.Network;
import cn.commonhelp.http_ansyn.utils.ThreadPoolUtils;
...
public class AnsynHttpRequest
{
			...
			boolean isCache, boolean isShowDialog)
	{
		...
		buffer.append(MainCommon.http.http_request_head)
		...
		if (map != null && map.size() > 0)
		{
			...
			for (String key : map.keySet())
			{
				...
				buffer.append(key).append("=")
						.append(Encoder.encode(map.get(key))).append("&");
			}
			buffer.append(value);
		}
		String requestUrl = buffer.toString();
		requestUrl = requestUrl.substring(0, requestUrl.length() - 1);
		LogMy.i("httpurl", requestUrl);
		...
		Network.WIFI:
			if (data != null && data.length > 0 && listData
					&& TimeCompare.compareHourTime(data[1]) > 3)
			{
				// if(data != null && data.length > 0 && listData &&
		...
		Network.NOWIFI: // 是
			if (data != null && data.length > 0 && listData
					&& TimeCompare.compareHourTime(data[1]) > 24 * 2)
			{
				// if(data != null && data.length > 0 && listData &&
				// TimeCompare.compareHourTime(data[1]) > 1){
				data = null;
		...
		if (isCache && data != null && data.length > 0)
		{ // 如果是缓存 取数据库缓存中数据
			callBack.back(data[0], url);
			return;
		}
		// 异步请求数据
		doAsynRequest(GET, null, context, callBack, requestUrl, isCache,
				isShowDialog, url);
	}
			...
			final boolean isCache, final boolean isShowDialog, final int intUrl)
	{
		...
		if (isShowDialog)
			((Activity) context).showDialog(1); // 加载对话框提示等待。。
		// 请求
		ThreadPoolUtils.execute(new MyRunnable(sendType, map, context,
				callBack, url, isCache, isShowDialog, intUrl));
	}
			...
			boolean isCache, boolean isShowDialog)
	{
		...
		LogMy.i("httpurl", requestUrl);
		// 异步请求数据
		doAsynRequest(POST, map, context, callBack, requestUrl, isCache,
				isShowDialog, url);
	}
			...
			boolean isCache, boolean isShowDialog)
	{
		...
		String requestUrl = buffer.toString();
		// requestUrl = requestUrl.substring(0,requestUrl.length()-1);
		LogMy.i("httpurl", requestUrl + map.toString());
		// 判断网络是否可用
...
class MyRunnable implements Runnable
{
	...
	public MyRunnable(final int sendType, final Map<String, String> map,
			final Context context, final ObserverCallBack callBack,
			final String url, final boolean isCache,
			final boolean isShowDialog, final int intUrl)
	{
	...
	public void run()
	{
		...
		try
		{
			...
			// String sUserAgent =
			// "code=yyyyMMddHHmmssSS,secretCode=anmaFromAndroid";
			...
			HttpConnectionParams.setConnectionTimeout(httpParams,
					AnsynHttpRequest.REQUEST_TIMEOUT);
			HttpConnectionParams.setSoTimeout(httpParams,
					AnsynHttpRequest.SO_TIMEOUT);
			...
			case AnsynHttpRequest.POST: // post 方式提交
				...
				LogMy.i("httpurl", url + map.toString());
		...
		{
			LogMy.e(AnsynHttpRequest.tag, e.getMessage());
			if (!isCache)
			{
				AnsynHttpRequest.sendBroadcastReceiverMessage(context,
						R.string.network_show_error);
