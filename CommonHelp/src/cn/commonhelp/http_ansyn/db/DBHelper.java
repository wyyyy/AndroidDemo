package cn.commonhelp.http_ansyn.db;
...
import cn.commonhelp.http_ansyn.entity.Store;
...
public class DBHelper extends SQLiteOpenHelper
{
	...
	public String[] getURLData(String url)
	{
		...
		try
		{
			...
			cursor = db
					.rawQuery(
							"select cacheData,createTime from cacheUrlData where url = ?",
							new String[]
							{ url });
	 ...
	 * @param jsonData
	 */
	@SuppressLint("SimpleDateFormat") public synchronized void addOrUpdateURLData(String url, String jsonData)
	{
		try
		{
			...
			{
				db.execSQL(
						"update cacheUrlData set cacheData =?, createTime = ? where url =? ",
						new String[]
						{ jsonData, jsonData, sdf.format(new Date()), url });
			} else
			{
				db.execSQL(
						"insert into cacheUrlData (url, cacheData, createTime) values(?, ?, ?)",
						new String[]
						{ url, jsonData, sdf.format(new Date()) });
			}
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	public boolean deleteURLData(String url)
	{
		...
		{
			db = getWritableDatabase(); // 获得数据库写对象\
			return db.delete("cacheUrlData", "url like ?", new String[]
			{ "%" + url + "%" }) > 0;
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	private synchronized boolean checkURLData(String url)
	{
		...
		try
		{
			...
			cursor = db.rawQuery(
					"select cacheData from cacheUrlData where url = ?",
					new String[]
					{ url });
			while (cursor.moveToNext())
			{
	 ...
	 * 
	 * @param store
	 * @return 创建时间：2012-9-21 下午2:07:03
	 */
	 ...
	 * 
	 * @param id
	 * @return 创建时间：2012-9-21 下午2:10:04
	 */
	public Store getStore(int id)
	{
		...
		try
		{
			...
			{
				byte[] tmpByte = cursor.getBlob(cursor
						.getColumnIndex("blobObj"));
	 ...
	 * @return
	 */
	@SuppressLint("SimpleDateFormat") public boolean addShouCang(Store store)
	{
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		try
		{
			...
			// db.execSQL("insert into shoucang (id, blobObj, time) values(?, ?, ?)",new
			// Object[]{store.id,
			// byteStream.toByteArray(),sdf.format(System.currentTimeMillis())});
		} catch (Exception e)
		{
			e.printStackTrace();
		} finally
	...
	public boolean deleteShouCang(String id)
	{
		try
		{
			...
			if (id == null)
				...
				return db.delete("shoucang", "id = ?", new String[]
				{ id }) > 0;
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	{
		String[] tables = context.getResources().getStringArray(0);
