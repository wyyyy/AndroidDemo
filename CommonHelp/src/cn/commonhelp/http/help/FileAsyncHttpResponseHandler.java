...
package cn.commonhelp.http.help;
		...
		AsyncHttpResponseHandler
{
	 ...
	 * @param append
	 *            whether data should be appended to existing file
	 * @param renameTargetFileIfExists
	 *            whether target file should be renamed if it already exists
	 */
	public FileAsyncHttpResponseHandler(File file, boolean append,
	 ...
	 */
	public FileAsyncHttpResponseHandler(File file, boolean append,
			boolean renameTargetFileIfExists, boolean usePoolThread)
	{
		super(usePoolThread);
		...
		if (file.isDirectory())
		{
			if (!file.mkdirs())
			{
				AsyncHttpClient.log
						.d(LOG_TAG,
								"Cannot create directories for requested Directory location, might not be a problem");
			}
		}
		this.file = file;
		this.append = append;
	...
	/**
	 * Retrieves File which represents response final location after possible
	 * renaming
	 * 
	...
	public File getTargetFile()
	{
		...
		{
			frontendFile = getOriginalFile().isDirectory() ? getTargetFileByParsingURL()
					: getOriginalFile();
		}
		return frontendFile;
	}
	...
	/**
	 * Will return File instance for file representing last URL segment in given
	 * folder. If file already exists and renameTargetFileIfExists was set as
	 * true, will try to find file which doesn't exist, naming template for such
	 * cases is "filename.ext" =&gt; "filename (%d).ext", or without extension
	 * "filename" =&gt; "filename (%d)"
	 * 
	 * @return File in given directory constructed by last segment of request
	 *         URL
	 */
	protected File getTargetFileByParsingURL()
	{
		Utils.asserts(getOriginalFile().isDirectory(),
				"Target file is not a directory, cannot proceed");
		Utils.asserts(getRequestURI() != null,
		...
		if (targetFileRtn.exists() && renameIfExists)
		{
			...
			{
				format = filename.substring(0, filename.lastIndexOf('.'))
						+ " (%d)"
			...
			while (true)
			{
				targetFileRtn = new File(getOriginalFile(), String.format(
						format, index));
