...
@SuppressLint("ViewHolder") public class MyAdapter extends BaseAdapter
{
	...
	public int getCount()
	{
		...
		if (status != null && status.getData() != null
				&& status.getData().getForecast() != null)
		{
	...
	public View getView(int postion, View view, ViewGroup praent)
	{
		...
		TextView txtTemperature = (TextView) iniView
				.findViewById(R.id.gcTxtTemperature);
		...
		rtTemperature.append(status.getData().getForecast()[postion].getLow()
				.substring(2).toString().trim());
		rtTemperature.append("~");
		rtTemperature.append(status.getData().getForecast()[postion].getHigh()
				.substring(2).toString().trim());
